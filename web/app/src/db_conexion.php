...
class db_conexion
{
  ...
  {
    $this->_settings = parse_ini_file(ROOT_PATH.'/config/database.config.ini');
    ...
    try {
      $this->_link = new mysqli(
        ...
        $this->_settings['db_username'],
        $this->_settings['db_password'],
        $this->_settings['db_name']);

      $this->_link->set_charset( $this->_settings['db_encode'] );
  ...
  public function filter( $data )
  {
    if( !is_array( $data ) )
    {
      $data = $this->_link->real_escape_string( $data );
  ...
  {
    $full_query = $this->_link->query( $query );
    if( $this->_link->error )
    {
      $this->log_db_errors( $this->_link->error, $query );
      return false;
    }
    else
    {
  ...
  {
    self::$counter++;
    $check = $this->_link->query( "SELECT 1 FROM $name" );
    if($check !== false)
    {
      if( $check->num_rows > 0 )
      {
  ...
  {
    self::$counter++;
    $num_rows = $this->_link->query( $query );
    if( $this->_link->error )
    {
      $this->log_db_errors( $this->_link->error, $query );
  ...
  public function get_results( $query, $object = false )
  {
    ...
    $results = $this->_link->query( $query );
    if( $this->_link->error )
    {
      $this->log_db_errors( $this->_link->error, $query );
      return false;
  ...
  public function insert( $table, $variables = array() )
  {
    ...
    {
      //return false;
      $this->log_db_errors( $this->_link->error, $sql );
      return false;
    }
    else
    {
   ...
   *
   * @access private
   * @return DB|db_conexion
   */
  static function getInstance()
  {
    if( self::$inst == null )
    {
      self::$inst = new db_conexion();
    }
    return self::$inst;
  }
